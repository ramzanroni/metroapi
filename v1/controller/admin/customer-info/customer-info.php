...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    try {
        ...
        if ($rowCount > 0) {
            ...
            $response->setSuccess(false);
            $response->addMessage("This phone Already exist in our database");
            $response->send();
            exit;
        }


        // insert contact-master 
        $picture = '';
        $code = '';
        ...
        $insertContact->bindParam(':bid', $bin_no, PDO::PARAM_STR);
        ...
        if ($rowCount > 0) {
            ...
            // updateCode
            $updateCode = $writeDB->prepare('UPDATE `contact_master` SET `code`=:code WHERE `id`=:last_id');
            $updateCode->bindParam(':code', $last_id, PDO::PARAM_STR);
            $updateCode->bindParam(':last_id', $last_id, PDO::PARAM_STR);
            $updateCode->execute();
            ...
            $insertDebtorStmt->bindParam(':debtorno', $last_id, PDO::PARAM_STR);
            $insertDebtorStmt->bindParam(':cm_id', $last_id, PDO::PARAM_STR);
            ...
            if ($rowCount === 1) {
                ...
                $insertBranchStmt->bindParam(':debtorno', $last_id, PDO::PARAM_STR);
    ...
    if ($rowCount == 0) {
        ...
        $returnArray['customer_info'] = $customerinfoArr;
        $response = new Response();
        $response->setHttpStatusCode(200);
        $response->toCache(true);
        $response->setSuccess(true);
    ...
    try {
        ...
        $debtorInfo = new DebtorMaster($jsonData->debtorno, '', $jsonData->name, $jsonData->address1, $jsonData->address2, $jsonData->address3, $jsonData->address4, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', $jsonData->phone1, '', $jsonData->email, '', '', $jsonData->updated_at, $jsonData->updated_by, '', '', '', '');
        $debtorno = $debtorInfo->getDebtorno();
        ...
        $updated_by = $debtorInfo->getUpdated_by();
        // check users 
        $checkUser = $readDB->prepare('SELECT * FROM contact_master WHERE id=:id');
        $checkUser->bindParam(':id', $debtorno, PDO::PARAM_STR);
        $checkUser->execute();
        ...
        // check phone 
        $checkPhone = $readDB->prepare('SELECT * FROM contact_master WHERE phone1=:phone1 AND id!=:id');
        $checkPhone->bindParam(':id', $debtorno, PDO::PARAM_STR);
        $checkPhone->bindParam(':phone1', $phone1, PDO::PARAM_STR);
        $checkPhone->execute();
        $rowCount = $checkPhone->rowCount();
        ...
        if ($rowCount > 0) {
            ...
            $response->setHttpStatusCode(404);
            $response->setSuccess(false);
            $response->addMessage("This phone number already used by someone");
            $response->send();
            exit;
        }
        // contact-master 
        $updateContactMaster = $writeDB->prepare('UPDATE `contact_master` SET `name`=:name,`address1`=:address1,`address2`=:address2,`address3`=:address3,`address4`=:address4,`phone1`=:phone1,`email`=:email,`updated_by`=:updated_by,`updated_at`=:updated_at WHERE `id`=:id');
        $updateContactMaster->bindParam(':name', $name, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':address1', $address1, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':address2', $address2, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':address3', $address3, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':address4', $address4, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':phone1', $phone1, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':email', $email, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':updated_at', $updated_at, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':updated_by', $updated_by, PDO::PARAM_STR);
        $updateContactMaster->bindParam(':id', $debtorno, PDO::PARAM_STR);
        $updateContactMaster->execute();
        $rowCount = $updateContactMaster->rowCount();
        if ($rowCount > 0) {
            // debtormaster 
            $query = "UPDATE debtorsmaster SET name=:name,address1=:address1,address2=:address2,address3=:address3,address4=:address4,phone1=:phone1,email=:email ,updated_at=:updated_at,updated_by=:updated_by WHERE debtorno=:debtorno";
            ...
            if ($rowCount == 1) {
                // update custbranch 
                $updateCustBranch = $writeDB->prepare('UPDATE `custbranch` SET `brname`=:brname,`braddress1`=:braddress1,`braddress2`=:braddress2,`braddress3`=:braddress3,`braddress4`=:braddress4, `phoneno`=:phoneno,`email`=:email WHERE `debtorno`=:debtorno');
                $updateCustBranch->bindParam(':brname', $name, PDO::PARAM_STR);
                $updateCustBranch->bindParam(':braddress1', $address1, PDO::PARAM_STR);
                $updateCustBranch->bindParam(':braddress2', $address2, PDO::PARAM_STR);
                $updateCustBranch->bindParam(':braddress3', $address3, PDO::PARAM_STR);
                $updateCustBranch->bindParam(':braddress4', $address4, PDO::PARAM_STR);
                $updateCustBranch->bindParam(':phoneno', $phone1, PDO::PARAM_STR);
                ...
                $rowCount = $updateCustBranch->rowCount();
                if ($rowCount > 0) {
                    $response = new Response();
                    $response->setHttpStatusCode(200);
                    $response->setSuccess(true);
