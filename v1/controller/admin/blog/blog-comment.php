...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    try {
        ...
        if (!isset($jsonData->reply) && !isset($jsonData->startData)) {
        ...
        if (isset($jsonData->startData)) {
            $startData = $jsonData->startData . " 00:00:00";
            $endDate = $jsonData->endDate . " 23:59:59";
            $blog_id = $jsonData->blog_id;
            $commentText = $jsonData->commentText;
            $mainQuery = "SELECT blogs_comments.id AS id, blogs_comments.blog_id AS blog_id, blog.title AS title, blogs_comments.comment AS comment, blogs_comments.ratings AS ratings, blogs_comments.user_id AS user_id, blogs_comments.status AS status, blogs_comments.create_at AS create_at, blogs_comments.blog_comment_id AS blog_comment_id, blogs_comments.parent AS parent, contact_master.name AS name FROM `blogs_comments` INNER JOIN contact_master ON contact_master.id=blogs_comments.user_id INNER JOIN blog ON blog.id=blogs_comments.blog_id WHERE ";
            if ($blog_id != '') {
                $subSql = "blogs_comments.blog_id=" . $blog_id . " AND ";
            }
            if ($commentText != '') {
                $textsearchQury = '';

                $searchKeywordList = explode(' ', $commentText);

                foreach ($searchKeywordList as $searchKey) {
                    $textsearchQury = $textsearchQury . " blogs_comments.comment LIKE '%" . $searchKey . "%' OR ";
                }
                $textsearchQury = rtrim($textsearchQury, 'OR ');
                $subSql = $subSql . $textsearchQury . "AND ";
            }

            $dateQuery = "blogs_comments.create_at>='$startData' AND blogs_comments.create_at<='$endDate' AND blogs_comments.parent=0 ORDER BY blogs_comments.create_at ASC";
            $finalQuery = $readDB->prepare($mainQuery . $subSql . $dateQuery);
            $finalQuery->execute();
            $rowCount = $finalQuery->rowCount();
            if ($rowCount > 0) {
                $comment = array();
                while ($rowComment = $finalQuery->fetch(PDO::FETCH_ASSOC)) {
    ...
    if (array_key_exists('blog_id', $_GET) && array_key_exists('page', $_GET)) {
        ...
        $end = ($page * $limit);
        $mainQuery = $readDB->prepare("SELECT blogs_comments.id AS id, blogs_comments.blog_id AS blog_id, blog.title AS title, blogs_comments.comment AS comment, blogs_comments.ratings AS ratings, blogs_comments.user_id AS user_id, blogs_comments.status AS status, blogs_comments.create_at AS create_at, blogs_comments.blog_comment_id AS blog_comment_id, blogs_comments.parent AS parent, contact_master.name AS name FROM `blogs_comments` INNER JOIN contact_master ON contact_master.id=blogs_comments.user_id INNER JOIN blog ON blog.id=blogs_comments.blog_id WHERE blogs_comments.blog_id=:blog_id AND blogs_comments.blog_comment_id>= :start AND blogs_comments.blog_comment_id<=:end AND blogs_comments.status=1 ORDER BY blogs_comments.create_at ASC");
    ...
    try {
        ...
        } elseif ($jsonData->type === 'status') {
            $id = $jsonData->id;
            $status = $jsonData->status;
            $user_id = $jsonData->user_id;
            $updateComment = $writeDB->prepare('UPDATE `blogs_comments` SET `user_id`=:user_id,`status`=:status WHERE `id`=:id');
            $updateComment->bindParam(':user_id', $user_id, PDO::PARAM_STR);
            $updateComment->bindParam(':status', $status, PDO::PARAM_STR);
            $updateComment->bindParam(':id', $id, PDO::PARAM_STR);
            $updateComment->execute();
            $rowCount = $updateComment->rowCount();
            if ($rowCount === 1) {
                // check reply
                ...
                $rowCount = $checkReply->rowCount();
                if ($rowCount != 0) {
                    $updateReply = $writeDB->prepare('UPDATE `blogs_comments` SET `user_id`=:user_id,`status`=:status WHERE `parent`=:parent');
                    $updateReply->bindParam(':user_id', $user_id, PDO::PARAM_STR);
                    $updateReply->bindParam(':status', $status, PDO::PARAM_STR);
                    $updateReply->bindParam(':parent', $id, PDO::PARAM_STR);
                    $updateReply->execute();
                    $rowCount = $updateReply->rowCount();
                    if ($rowCount != 0) {
                        $response = new Response();
                        $response->setHttpStatusCode(200);
                        $response->setSuccess(true);
                        $response->addMessage("Comment and reply status change success");
                        $response->send();
                        exit();
                    }
                } else {
                    ...
                    $response->setHttpStatusCode(200);
                    $response->setSuccess(true);
                    $response->addMessage("Comment status change success");
                    $response->send();
                    exit();
                }
            } else {
